...
void compute_energy_matrix(const Image* img, Matrix* energy) {
  ...
  for (int i = 1; i < Image_height(img) - 1; ++i)
  {
      for (int j = 1; j < Image_width(img) - 1; ++j)
      {
          ...
          int energyPixel = squared_difference(south, north) + 
                            squared_difference(east, west);
          *Matrix_at(energy, i, j) = energyPixel;
          if (energyPixel > max)
          {
              max = energyPixel;
          }
...
void compute_vertical_cost_matrix(const Matrix* energy, Matrix *cost) {
    ...
    for (int i = 1; i < Matrix_height(cost); ++i)
    {
        for (int j = 0; j < Matrix_width(cost); ++j)
        {
            ...
            int costTotal = *Matrix_at(energy, i,j) + 
                            Matrix_min_value_in_row(cost, i - 1, startCol, endCol);
            *Matrix_at(cost, i, j) = costTotal;       
        }
    }
...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
    ...
    for (int i = Matrix_height(cost)-2; i >= 0; --i)
    {
            ...
            seam[i] = Matrix_column_of_min_value_in_row(cost, i, startCol, endCol);
    }
}
...
void remove_vertical_seam(Image *img, const int seam[]) {       
    ...
    for (int i = 0; i < Image_height(newImage); ++i)
    {
        for (int j = 0; j < seam[i]; ++j)
        {
            Image_set_pixel(newImage, i, j, Image_get_pixel(img, i, j));
        }
        for (int k = seam[i] + 1; k < Image_width(img); ++k)
        {
            Image_set_pixel(newImage, i, k - 1, Image_get_pixel(img, i, k));
        }
    }
    *img = *newImage;
    delete newImage;
